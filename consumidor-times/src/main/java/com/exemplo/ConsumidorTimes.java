...
public class ConsumidorTimes {
    private static final String QUEUE_NAME = "team_queue";
    private Connection connection;
    private Channel channel;
    private ObjectMapper objectMapper;
    private IdentificadorTimes identificador;
    private AtomicLong mensagensProcessadas;

    public ConsumidorTimes() {
        this.objectMapper = new ObjectMapper();
        this.identificador = new IdentificadorTimes();
    ...
    public void conectarRabbitMQ() throws IOException, TimeoutException {
        ...
        for (int i = 0; i < maxTentativas; i++) {
            try {
                ...
                System.out.println("Conectado ao RabbitMQ - Consumidor de Identificação de Times");
                return;
            } catch (Exception e) {
    ...
    public void iniciarConsumo() throws IOException {
        ...
        System.out.println("Identificação de times de futebol com biblioteca Smile ML");
        ...
        DeliverCallback deliverCallback = (consumerTag, delivery) -> {
            try {
                ...
                // Processar com IA PURAMENTE VISUAL
                IdentificadorTimes.ResultadoIdentificacao resultado = identificador.identificarTime(mensagem);
                
                long tempoProcessamento = System.currentTimeMillis() - inicio;
                long count = mensagensProcessadas.incrementAndGet();
                
                String emoji = resultado.getCodigoTime().equals("DESCONHECIDO") ? "❓" : "⚽";
                
                System.out.println(String.format("%s RESULTADO: %s - %s", 
                    emoji, resultado.getCodigoTime(), resultado.getNomeCompleto()));
                ...
                if (count % 10 == 0) {
                    System.out.println(String.format("=== Estatísticas: %d brasões analisados ===", count));
                }
    ...
    public static void main(String[] args) {
        ConsumidorTimes consumidor = new ConsumidorTimes();
        
        Runtime.getRuntime().addShutdownHook(new Thread(() -> {
            System.out.println("\nFinalizando consumidor de identificação de times...");
            consumidor.fechar();
        }));
